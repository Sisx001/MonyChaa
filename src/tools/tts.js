...
async function speak(text) {
  if (config.key('ELEVENLABS_API_KEY')) {
    const res = await fetch('https://api.elevenlabs.io/v1/text-to-speech/21m00Tcm4TlvDq8ikWAM', {
      method: 'POST',
      headers: { 'xi-api-key': config.key('ELEVENLABS_API_KEY'), 'Content-Type': 'application/json' },
  ...
  if (config.key('OPENAI_API_KEY')) {
    const res = await fetch('https://api.openai.com/v1/audio/speech', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${config.key('OPENAI_API_KEY')}`, 'Content-Type': 'application/json' },
...
function available() {
  return Boolean(config.key('ELEVENLABS_API_KEY') || config.key('OPENAI_API_KEY'));
}
