...
- **Human behavior** (Settings → Generation → Human behavior) — makes replies read like a person texting, not an assistant. Two layers kept in lockstep:
  ...
  - **Adaptive reply length** — scales the target reply length to the incoming message: a one-word "ok" won't get a paragraph, and a detailed multi-question message gets a fuller answer. Never exceeds your configured max length, and an explicit per-contact max always wins.
  - **Per-contact mood baseline** — beyond the current message, the bot learns each contact's *typical* emotional pattern from their recent history (e.g. someone who usually reaches out stressed) and carries a standing baseline into the prompt, so it starts on the right foot even before reading the new message. Only kicks in when there's a real pattern (≥3 messages and ≥30% of a clear non-neutral mood).
